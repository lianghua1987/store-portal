...
var category = {OBJ: $("#_JD_ALLSORT"),
		...
		URL_Serv: "http://127.0.0.1:8081/api/item/categories?callback=category.getDataService",
        ...
        return "" == c ? d : (/^http[s]?:\/\/([\w-]+\.)+[\w-]+([\w-.\/?%&=]*)?$/.test(c) || (c = c.replace(/-000$/, ""), c = c.match(/^\d*-\d*$/) ? "http://channel.jd.com/" + c + ".html" : "http://localhost:8082/" + c + ".html"), '<a href="' + c + '">' + d + "</a>")
